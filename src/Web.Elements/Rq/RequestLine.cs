...
public sealed class RequestLine : RqWrap, IRqRequestLine
{
    ...
    public RequestLine(IRequest origin) : base(origin)
    {
    }
    ...
    {
        return Trimmed(
            Match(this.Line()).Groups[TokenValue(token)].Value,
            token
        );
    }
